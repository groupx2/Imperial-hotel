  ...
  $scope.showBooking = function() {
    ...
    $http({
      ...
      response.data.data.data.forEach(item => {
        ...
        <img class="card-img-top" src="${url}/img/roomCategories/${item.room.roomCategory.photo}" alt="Card image cap" style="width:100%;height:55%">
        <div class="card-body">
            ...
            <p class="card-title">${item.room.checkIn.split('T')[0]} → ${item.room.checkOut.split('T')[0]}</p>
        </div>
      </div>
        `)($scope));
